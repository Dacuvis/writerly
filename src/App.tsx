...
function App() {
  ...
  useEffect(() => {
    fetch(apiUrl("/api/manuscripts"))
      ...
      .then(async (items: ManuscriptSummary[]) => {
        ...
        if (items[0]) await loadManuscript(items[0].id);
        else setSaveStatus("saved");
      })
      .catch(() => setSaveStatus("error"));
  }, []);
  const selectedChapter =
    manuscript?.chapters.find((chapter) => chapter.id === selectedId) ?? null;
  ...
  async function saveChapter(
    ...
    try {
      ...
      const saved = (await response.json()) as Chapter;
      updateChapter({
        ...saved,
  ...
  function handleShareAsPdf() {
    ...
    setIsExporting(true);
    try {
      void exportChapterPdf(selectedChapter, manuscript?.title ?? "").finally(
  ...
  async function createManuscript(data: {
    title: string;
    description: string;
  }) {
    ...
    if (!response.ok) throw new Error("Could not create manuscript");
    const created = (await response.json()) as ManuscriptSummary;
    setManuscripts((items) => [created, ...items]);
    await loadManuscript(created.id);
  }
  async function renameManuscript(data: {
    title: string;
    description: string;
  }) {
    ...
    if (!response.ok) throw new Error("Could not rename manuscript");
    const saved = (await response.json()) as ManuscriptSummary;
    setManuscripts((items) =>
      items.map((item) => (item.id === saved.id ? saved : item)),
    );
    setManuscript((item) => item && { ...item, ...saved });
  }
  async function deleteManuscript() {
    ...
    setManuscript(null);
    setSelectedId(null);
    if (remaining[0]) await loadManuscript(remaining[0].id);
  }
  async function createChapter(title: string) {
    ...
    if (!response.ok) throw new Error("Could not create chapter");
    const created = (await response.json()) as Chapter;
    setManuscript(
      (current) =>
        current && { ...current, chapters: [...current.chapters, created] },
    );
  ...
  async function renameChapter(title: string) {
    ...
    if (!response.ok) throw new Error("Could not rename chapter");
    updateChapter({ ...selectedChapter, title });
  }
  async function deleteChapter() {
    ...
    setManuscript({ ...manuscript, chapters });
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <div className="crumbs">
            <span>{manuscript?.title ?? "No manuscript"}</span>
            <i>/</i>
            <strong>{selectedChapter?.title ?? ""}</strong>
          </div>
          <div className="top-actions">
            <span className={`saved ${saveStatus}`}>
              <b></b>
              {saveStatus === "saving"
                ? "Saving…"
                : saveStatus === "error"
            ...
            <button className="share-button" onClick={handleShareAsPdf} disabled={isExporting}>
              {isExporting ? "Exporting…" : "Share"} <span>↗</span>
            </button>
          </div>
        </header>
        {selectedChapter ? (
          <Editor chapter={selectedChapter} onSave={saveChapter} />
        ) : (
          <div className="empty-state">
            ...
            <p>
              {manuscript
                ...
                : "Keep every story and draft in one calm place."}
            </p>
            <button
              className="new-button"
              onClick={() =>
                setDialog(manuscript ? "new-chapter" : "new-manuscript")
              }
            >
              + {manuscript ? "Add chapter" : "New manuscript"}
            </button>
          </div>
        )}
      </section>
      {dialog === "new-manuscript" && (
        <ManuscriptDialog
          mode="create"
          onClose={() => setDialog(null)}
          onSubmit={createManuscript}
        />
      )}
      {dialog === "rename-manuscript" && manuscript && (
        <ManuscriptDialog
          mode="rename"
          initialTitle={manuscript.title}
          initialDescription={manuscript.description}
          onClose={() => setDialog(null)}
          onSubmit={renameManuscript}
        />
      )}
      {dialog === "delete-manuscript" && manuscript && (
        <ConfirmDialog
          title="Delete manuscript?"
          description={`“${manuscript.title}” and every chapter inside it will be permanently deleted.`}
      ...
      {dialog === "rename-chapter" && selectedChapter && (
        <ChapterDialog
          mode="rename"
      ...
      {dialog === "delete-chapter" && selectedChapter && (
        <ConfirmDialog
          title="Delete chapter?"
          description={`“${selectedChapter.title}” will be permanently deleted.`}
