...
function ChapterNotes({ chapterId }: { chapterId: string }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/chapters/${chapterId}`).then((response) => response.ok ? response.json() : Promise.reject()).then((data: { notes?: Note[] }) => {
      if (active) setNotes(data.notes ?? []);
    }).catch(() => { if (active) setNotes([]); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [chapterId]);
  ...
  async function addNote(event: React.FormEvent) {
    ...
    setSaving(true);
    try { const response = await fetch(`/api/chapters/${chapterId}/notes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ body }) }); if (!response.ok) throw new Error(); const note = await response.json() as Note; setNotes((items) => [...items, note]); setDraft(""); } finally { setSaving(false); }
  }
  async function saveNote(id: string, body: string) {
    const value = body.trim(); if (!value) return;
    const response = await fetch(`/api/notes/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ body: value }) });
    if (response.ok) { const saved = await response.json() as Note; setNotes((items) => items.map((note) => note.id === id ? saved : note)); }
  }
  async function deleteNote(id: string) {
    const response = await fetch(`/api/notes/${id}`, { method: "DELETE" });
    if (response.ok) setNotes((items) => items.filter((note) => note.id !== id));
  }

  return <section className="notes"><div className="notes-heading"><span>✦</span><strong>Notes</strong><small>{notes.length}</small></div>{loading ? <p>Loading notes…</p> : <><div className="note-list">{notes.map((note) => <div className="note-card" key={note.id}><textarea aria-label="Chapter note" defaultValue={note.body} onBlur={(event) => void saveNote(note.id, event.target.value)} rows={3} /><button type="button" aria-label="Delete note" title="Delete note" onClick={() => void deleteNote(note.id)}>×</button></div>)}</div>{!notes.length && <p>Keep plot ideas, research, or reminders for this chapter here.</p>}<form className="note-form" onSubmit={(event) => void addNote(event)}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Add a note…" rows={3} maxLength={5000} /><button type="submit" disabled={!draft.trim() || saving}>{saving ? "Saving…" : "Add note"}</button></form></>}</section>;
}
